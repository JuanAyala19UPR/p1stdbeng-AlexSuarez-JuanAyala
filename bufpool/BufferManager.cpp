...
BufferManager::~BufferManager() {
    // Best effort: a destructor cannot report flush failures anywhere
    // useful, but descriptors must not leak and dirty pages deserve a
    // chance to reach disk.
    std::string error;
    close_all(error);
}
...
bool BufferManager::open_file(const std::string& path, bufman::File*& file, std::string& error) {
    ...
    files_.emplace_back(std::move(opened));
    file = &files_.back();
    return true;
}
...
// then the policy's pick among the page-holding, unpinned frames. With no
// unpinned page-holding frame at all, nothing can be evicted ("all frames
// pinned"). The policy's answer is validated here so a broken policy can
// never make the manager index out of range or evict a live page.
bool BufferManager::find_victim_frame(std::size_t& frame, std::string& error) const {
    ...
    const std::optional<std::size_t> pick = policy_->pick_victim(candidates);
    if (!pick || *pick >= frame_page_.size() || !frame_page_[*pick] ||
        pool_.frame(*pick).pin_count != 0) {
        error = "replacement policy returned an invalid victim";
        return false;
    }
    frame = *pick;
    return true;
...
bool BufferManager::pin(bufman::File& file, std::uint64_t block_number,
    ...
    std::size_t victim = 0;
    if (!find_victim_frame(victim, error)) {
        return false;
    }
    frame = victim;
...
bool BufferManager::alloc_page(bufman::File& file, std::uint64_t block_number,
    ...
    // Blocks are allocated densely: a page beyond the end of file is only
    // allowed if every block between the disk's end and the request is a
    // fresh page still sitting in the pool (allocated here, not yet
    // flushed). Anything else would write past a zero-filled gap and break
    // the whole-blocks invariant record_count relies on.
    if (block_number < blocks) {
        error = "block already exists on disk";
        return false;
    }
    for (std::uint64_t gap = blocks; gap < block_number; ++gap) {
        if (!page_table_.count(PageKey{file.fd, gap})) {
            error = "block is not the next block after the end of file";
            return false;
        }
    }
...
bool BufferManager::close_all(std::string& error) {
    // Flush failures are reported through `error` and the return value, but
    // teardown continues: the file descriptors are going away either way.
    const bool flushed = flush_all(error);
    for (bufman::File& file : files_) {
        bufman::close(file);
    }
    files_.clear();
