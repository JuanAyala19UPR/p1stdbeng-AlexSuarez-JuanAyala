...
// The on-disk layout stores native 4-byte ints in native byte order, so it
// is only portable between hosts that agree on both. The static_assert
// pins the size assumption down at compile time.
static_assert(sizeof(int) == 4, "the on-disk format assumes 32-bit int");
